...
└───────────────────────────────────────────────────────────┘
~~~~
{: #arch- title="Security Context Establishment and Propagation"}

One example of workload security context establishment and propagation is as follows:

1. Workload 1 requests to load the LLM(Large Language Model) within Workload 2; Workload 2 requires that Workload 1 must have a hardware-based security mechanism to prevent Model leakage. The hardware-based security context information of Workload 1 can be obtained by running a remote attestation protocol. Workload 1 provides its Identity and the security context information to Workload 2 to apply for loading the LLM.

2. Workload 2 verifies the Identity and corresponding security context information provided by Workload 1 to confirm whether the LLM can be provided to Workload 1.

3. Workload 1 also requires the LLM on Workload 2 to use the vertical industry domain-specific data on Workload 3 for Model fine-tuning.

4. Workload 2 provides Workload 1's Identity and the hardware-based security context information of Workload 1, as well as Workload 2’s Identity to Workload 3.

5. Workload 3 uses that information for authentication and, upon passing, provides the vertical industry domain-specific data to Workload 2 for model fine-tuning.

6. After fine-tuning the LLM using the vertical industry domain-specific data provided by Workload 3, Workload 2 provides the LLM to Workload 1 for running.
